...
from google.adk.agents import Agent
from google.adk.tools.toolbox_tool import MCPToolset, HttpServerParameters

# POSTGRES_MCP_HEADERS = {
#     "Authorization": "Bearer your-secure-token",  # 如果需要鉴权
...
Postgres_MCP_Agent = Agent(
    model="gemini-2.0-flash",
    name="Postgres_MCP_Agent",
    instruction=POSTGRES_PROMPT,
    tools=[
        MCPToolset(
            connection_params=HttpServerParameters(
                url="http://127.0.0.1:5000",  # 你的 MCP Toolbox Server 地址
                # headers=POSTGRES_MCP_HEADERS
            )
        )
    ],
)
